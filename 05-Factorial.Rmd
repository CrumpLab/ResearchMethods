...
The effect of one independent variable can depend on the level of the other in several different ways. This is shown in Figure \@ref(fig:c5ibar).

```{r c5ibar, fig.cap="Bar Graphs Showing Three Types of Interactions. In the top panel, one independent variable has an effect at one level of the second independent variable but not at the other. In the middle panel, one independent variable has a stronger effect at one level of the second independent variable than at the other. In the bottom panel, one independent variable has the opposite effect at one level of the second independent variable than at the other.",echo=FALSE,fig.env='png'}
knitr::include_graphics("Figures/C8interactionbars.pdf")
```


In the top panel, independent variable “B” has an effect at level 1 of independent variable “A” but no effect at level 2 of independent variable “A.” (This is much like the study of Schnall and her colleagues where there was an effect of disgust for those high in private body consciousness but not for those low in private body consciousness.) In the middle panel, independent variable “B” has a stronger effect at level 1 of independent variable “A” than at level 2. This is like the hypothetical driving example where there was a stronger effect of using a cell phone at night than during the day. In the bottom panel, independent variable “B” again has an effect at both levels of independent variable “A,” but the effects are in opposite directions. Figure \@ref(fig:c5ibar) shows the strongest form of this kind of interaction, called a crossover interaction. One example of a crossover interaction comes from a study by Kathy Gilliland on the effect of caffeine on the verbal test scores of introverts and extraverts [@gilliland_interactive_1980]. Introverts perform better than extraverts when they have not ingested any caffeine. But extraverts perform better than introverts when they have ingested 4 mg of caffeine per kilogram of body weight. Figure \@ref(fig:c5iline) shows examples of these same kinds of interactions when one of the independent variables is quantitative and the results are plotted in a line graph.

```{r c5iline, fig.cap="Line Graphs Showing Three Types of Interactions. In the top panel, one independent variable has an effect at one level of the second independent variable but not at the other. In the middle panel, one independent variable has a stronger effect at one level of the second independent variable than at the other. In the bottom panel, one independent variable has the opposite effect at one level of the second independent variable than at the other.",echo=FALSE,fig.env='png'}
knitr::include_graphics("Figures/C8interactionlines.pdf")
```
...
NoShoes_NoHat<-round(rnorm(10,57,1)+0+0)
Shoes_NoHat<-NoShoes_NoHat+1
NoShoes_Hat<-NoShoes_NoHat+6
Shoes_Hat<-NoShoes_NoHat+7
df<-data.frame(NoShoes_NoHat,Shoes_NoHat,NoShoes_Hat,Shoes_Hat)
kable(df)
...
To find the main effect of the shoes manipulation we want to find the mean height in the no shoes condition, and compare it to the mean height of the shoes condition. To do this, we \emph{collapse}, or average over the observations in the hat conditions. For example, looking only at the no shoes vs. shoes conditions we see the following averages for each subject.

```{r, echo=FALSE}
df<-data.frame(NoShoes=(NoShoes_NoHat+NoShoes_Hat)/2,
               Shoes=(Shoes_NoHat+Shoes_Hat)/2)
kable(df)
```
...
```{r, echo=FALSE}
df<-data.frame(NoHat=(NoShoes_NoHat+Shoes_NoHat)/2,
               Hat=(NoShoes_Hat+Shoes_Hat)/2)
kable(df)
```
...
```{r,echo=FALSE}
df<-data.frame(heights=c(NoShoes_NoHat,Shoes_NoHat,NoShoes_Hat,Shoes_Hat),
               Wearing_Shoes=rep(rep(c("No_Shoes","Shoes"),each=10),2),
               Wearing_Hat=rep(c("No_Hat","Hat"),each=20))
df_means<-ddply(df,.(Wearing_Shoes,Wearing_Hat),summarize,mean_height=mean(heights))
ggplot(df_means,aes(x=Wearing_Shoes,y=mean_height, group=Wearing_Hat,color=Wearing_Hat,fill=Wearing_Hat))+
  geom_bar(stat="identity",position="dodge")+theme_classic(base_size=14)+
...
df_means<-ddply(df,.(Wearing_Shoes),summarize,mean_height=mean(heights))
ggplot(df_means,aes(x=Wearing_Shoes,y=mean_height))+
  geom_bar(stat="identity",position="dodge")+theme_classic(base_size=14)+
  coord_cartesian(ylim=c(55,65))

df_means<-ddply(df,.(Wearing_Hat),summarize,mean_height=mean(heights))
ggplot(df_means,aes(x=Wearing_Hat,y=mean_height))+
  geom_bar(stat="identity",position="dodge")+theme_classic(base_size=14)+
  coord_cartesian(ylim=c(55,65))
```
...
A1B2<-rnorm(1,17,0)
A2B2<-rnorm(1,13,0)
df<-data.frame(DV=c(A1B1,A2B1,A1B2,A2B2),
               IV1=rep(rep(c("Level_1","Level_2")),2),
               IV2=rep(c("Level_1","Level_2"),each=2))
ggplot(df,aes(y=DV, x=IV1, group=IV2,color=IV2,fill=IV2))+
  geom_bar(stat="identity",position="dodge")+theme_classic(base_size=14)
...
dvs<-c(1,2,3,4,
       ...
       1,3,1,3,
       1,3,3,1)
df<-data.frame(DV=dvs,
               IV1=rep(rep(rep(c("Level_1","Level_2")),2),4),
               IV2=rep(rep(c("Level_1","Level_2"),each=2),4),
               Experiment=rep(c("IV1&IV2","IV2","IV1","IV1xIV2"),each=4))
ggplot(df,aes(y=DV, x=IV1, group=IV2,color=IV2,fill=IV2))+
  geom_bar(stat="identity",position="dodge")+theme_classic(base_size=14)+
...
ggplot(df,aes(y=DV, x=IV1, group=IV2,color=IV2,fill=IV2))+
  geom_line()+
  geom_point()+
  theme_classic(base_size=14)+facet_wrap(~Experiment)
...
This approach is often used to assess the validity of new psychological measures. For example, when John Cacioppo and Richard Petty created their Need for Cognition Scale—a measure of the extent to which people like to think and value thinking—they used it to measure the need for cognition for a large sample of college students, along with three other variables: intelligence, socially desirable responding (the tendency to give what one thinks is the “appropriate” response), and dogmatism [@cacioppo_need_1982]. The results of this study are summarized in Figure \@ref(fig:c5need), which is a correlation matrix showing the correlation (Pearson’s r) between every possible pair of variables in the study.

```{r c5need, fig.cap="Correlation Matrix Showing Correlations Among the Need for Cognition and Three Other Variables Based on Research by Cacioppo and Petty (1982)",echo=FALSE,fig.env='png'}
knitr::include_graphics("Figures/C8need.pdf")
```
